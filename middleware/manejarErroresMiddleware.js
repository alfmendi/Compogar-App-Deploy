...
const manejarErroresMiddleware = (err, req, res, next) => {
  ...
  // Se genera porque en el modelo existe unique:true
  if (err.code && err.code === 11000) {
    if (err.message.includes("proyectoHotel.empleados")) {
      errorPersonalizado.mensaje = `Ya existe un cliente con ese ${Object.keys(
        err.keyValue
      )}`;
    }
    // Si existe otro/s modelos que tambien puedan generar ese error,
    // se debería añadir otra/s condicion/es
    errorPersonalizado.codigoEstado = 400;
  }
